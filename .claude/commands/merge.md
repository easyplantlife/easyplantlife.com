...
4. **Review code changes**
   ...
   - Analyze changes for:
     ...
     - Brand guideline adherence (calm, minimal UI)
...
7. **Local validation (optional but recommended)**
   ...
   - Run full validation suite:
     - `npm ci` (ensure dependencies are up to date)
     - `npm run lint`
     - `npm test`
     - `npm run build`
...
- **CI must pass**: All status checks must pass before merging.
- **Squash merge default**: Squashing keeps the main branch history clean.
- **Delete branches**: Always delete feature branches after merge to keep repository clean.
- **Issue linking**: Always link PRs to issues using "Closes #N" or "Resolves #N" in PR description.
- **TDD compliance**: Ensure tests were written before implementation per project guidelines.
